// ZeroG Toolbox — Grainy Gradient / Noise Texture Generator
// Build layered noise-over-gradient CSS/SVG backgrounds (2026 "grainy gradient" trend)
...
function generateGradient(width, height, colors) {
  ...
  if (colors.length === 2) {
    ...
    // Three-color gradient (triangular/radial)
...
function generateCSS(presetId, noiseOpacity = 0.3) {
  ...
  // Generate base64 noise texture for CSS background-image
  ...
  // Construct CSS with noise overlay using ::after pseudo-element
  const cssCode = `/* Grainy Gradient Background — ZeroG Toolbox */
...
function generateSVG(presetId, noiseOpacity = 0.3) {
  ...
  // Build SVG with gradient and feTurbulence filter for grain effect
  let gradientStops = '';
  if (preset.colors.length >= 2) {
...
async function handleGeneratePreview() {
  ...
  try {
    ...
    // Display result on preview image element
...
async function handleExportCSS() {
  ...
  try {
    ...
    // Display CSS in code output area and copy to clipboard
...
document.addEventListener('DOMContentLoaded', () => {
  // Populate preset options from GRADIENT_PRESETS
  const presetSelect = document.getElementById('grainy-gradient-preset-select');
  if (presetSelect) {
