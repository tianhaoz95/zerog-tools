// ZeroG Toolbox — Silence Remover & Auto-Trim for Podcasts
// Analyzes waveform amplitude to detect silent gaps, auto-cuts them below a threshold
...
function startSilenceRemoval(options) {
  ...
  isProcessing = true;
  updateStatus('Starting silence detection...', 'info');
  ...
  if (!worker) {
    worker = new Worker('/src/silence-remover.worker.js');
...
function handleWorkerMessage(e) {
  ...
  if (type === 'status') {
    ...
    updateStatus(message || 'Silence removal complete!', 'success');
  } else if (type === 'error') {
    isProcessing = false;
...
function renderCleanedAudio(audioData, sampleRate) {
  ...
  const cleanedBuffer = new OfflineAudioContext(1, audioData.length, sampleRate);
  const channelData = cleanedBuffer.getChannelData(0);
  ...
  window.cleanedAudioBuffer = cleanedBuffer.decodeAudioData().then(buffer => {
    return buffer;
  }).catch(err => {
...
function showPlaybackControls() {
  const controls = document.getElementById('silence-controls');
  if (!controls) return;
...
function updateStatus(message, type) {
  const banner = document.getElementById('silence-status');
  if (!banner) return;

  banner.textContent = message;
  banner.style.display = 'block';
  banner.className = `silence-banner silence-banner--${type}`;
}
...
function handleFileUpload() {
  const fileInput = document.getElementById('silence-file-input');
  const fileNameDisplay = $('#silence-filename-display');
  ...
  if (fileInput.files.length > 0) {
    ...
    loadAudioFile(file).then(buffer => {
      updateStatus('Audio loaded. Configure threshold and click "Process".', 'success');
    }).catch(err => {
...
function handleGenerate() {
  const options = {
    silenceThreshold: parseFloat($('#silence-threshold').value) || -40,
    minSilenceDuration: parseFloat($('#silence-min-duration').value) || 0.5
  };

  startSilenceRemoval(options);
}
...
function resetSilenceToolState() {
  isProcessing = false;
  ...
  // Clear status banner
  const statusBanner = document.getElementById('silence-status');
  if (statusBanner) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Generate button
  const btnGenerate = document.getElementById('btn-silence-generate');
  if (btnGenerate) {
    btnGenerate.addEventListener('click', handleGenerate);
  }

  // Back button
  const btnBack = document.getElementById('btn-silence-remover-back');
  if (btnBack) {
...
window.resetSilenceToolState = resetSilenceToolState;